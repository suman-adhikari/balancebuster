...
interval=argumentList[2] if len(argumentList) > 2 else None
dateFrom=argumentList[3] if len(argumentList) > 3 else None
dateTo=argumentList[4] if len(argumentList) > 4 else None

#print dateTo
#sys.exit()


exportRepo = ExportRepo("localhost", "root", "", "dbbalancebuster","utf8")
data = exportRepo.GetData(interval,dateFrom,dateTo)
...
chart_line.add_series({
    ...
    'marker': {'type': 'circle','size': 3,'color':'blue'},
...
worksheet.insert_chart('D5', chart_line,  {'x_scale': 2.3, 'y_scale': 1.5} )
worksheet.insert_chart('D35', chart_column,  {'x_scale': 2.3, 'y_scale': 1.5} )
